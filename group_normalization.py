...
def group_norm(x, G, eps=1e-5):
    ...
    init = tf.truncated_normal(shape=[C], mean=0.0, stddev=1.0, dtype=tf.float32)
    gamma = tf.Variable(init, dtype=tf.float32, name='gamma')
    beta = tf.Variable(init, dtype=tf.float32, name='beta')
...
def create_conv_layer(input_data, num_input_channels, num_filters, variable_scope_name, activation='linear'):
    ...
    if variable_scope_name == 'conv_layer_2':
        for i in range(num_filters):
            tf.summary.histogram('before'+str(i), out_layer[:,:,:,i])
            mean, var = tf.nn.moments(out_layer[:,:,:,i],axes=[0,1,2])
            tf.summary.scalar('mean'+str(i), mean)
            tf.summary.scalar('var'+str(i), var)
    
    if hp.gn_conv == True:
        out_layer = group_norm(out_layer, hp.num_groups)
    ...
    if variable_scope_name == 'conv_layer_2':
        for j in range(num_filters):
            tf.summary.histogram('after'+str(j), out_layer[:,:,:,j])
...
def train_network(train_x, train_y, test_x, test_y):
    ...
    with tf.name_scope("compute_gradients"):
        ...
        grads, params = list(zip(*grads_and_params))
        for var in grads:
            tf.summary.histogram(var.name, var)
        tf.summary.scalar('l-2_grad',tf.global_norm(grads))
        ...
        for epoch in range(hp.num_training_epochs):
            ...
            for itr in range(num_batches):
                ...
                if iteration_count%10 == 0:
                    train_accuracy = sess.run(accuracy, feed_dict={network_input:train_x[0:10000], network_output:train_y[0:10000]})
...
#                     print("Epoch :", epoch, "iteration :", iteration_count)
    ...
    np.save('train_acc_list_LR='+str(hp.learning_rate)+',GN='+str(hp.num_groups)+'.npy', train_accuracy_list)
    np.save('test_acc_list_LR='+str(hp.learning_rate)+',GN='+str(hp.num_groups)+'.npy', test_accuracy_list)
    np.save('train_loss_list_LR='+str(hp.learning_rate)+',GN='+str(hp.num_groups)+'.npy', train_loss_list)
    np.save('iter_list_LR='+str(hp.learning_rate)+',GN='+str(hp.num_groups)+'.npy', iteration_list)
